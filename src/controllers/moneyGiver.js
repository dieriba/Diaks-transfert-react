...
//RENDER THE SEARCH FOR MEDIUM ADMIN SO THEY CAN LOOK FOR SPECIFIC TRANSFERT WITH A GIVEN ID
const searchTransfertPage = (req, res, next) => {
	const user = req.session.user;
	try {
		res.status(200).render('users/regular-user/search-clients', {
			title: 'Diak\'s Project - search',
			user,
		});
	} catch (error) {
		next(error);
...
const validateTransfert = async (req, res, next) => {
	try {
		...
		await Transfert.findOneAndUpdate(
			{ _id: id },
			{ hasTakeMoney: true, contactNumber: contactNumber , payoutDay : new Date() },
			{
				new: true,
				runValidators: true,
			}
